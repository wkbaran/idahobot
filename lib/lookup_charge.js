...
ChargeLookup.prototype.states = {
  ...
  when: { // expecting input to be a date
    prompt:"What was the date of the charge?",
    next:'amount'
  },
  amount: {  // expecting currency
    prompt:"What was the amount of your charge?",
    next:'finished'
  },
  finished: {}
...
ChargeLookup.prototype.commands = {
  RESET:{},
  CANCEL:{},
  HELP:{}
};
...
ChargeLookup.prototype.handleEvent = function ( 
  ...
  if (event.message && event.message.text) {
    ...
    if ( event.message.text.toUpperCase() === "RESET" ) { 
      ...
      console.log("Reset lookup_charge session",session);
      finished(session);
      return;
    } else
