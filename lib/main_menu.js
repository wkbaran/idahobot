...
MainMenu.prototype.handleEvent = function ( 
  ...
  if ( event.postback && event.postback.payload ) {
    if ( this.VALID_PLUGINS[event.postback.payload] ) {
      ...
      var ctx = this;
      mod.handleEvent(event,session,req,res,
        function ( session ) {
          finished(session);
          ctx.messenger.sendTextMessage(
  ...
  if ( event.message && event.message.text ) {
    ...
    if ( event.message.text.toUpperCase() === "HELP" ) {
...
/**
 * Get basic info about a user from Facebook
 * @param {integer} uid User id
 */
function getUserInfo ( uid, cb ) {
  var graph = require('fbgraph');
  graph.post(userId+"/feed?access_token="+this.token);
